...
namespace WebSite.Models
{
    public class CommonData
    {
        ...
        {
            return new List<string> { "Thanh toán cuối tháng", "Thanh toán đầu nhận","Thanh toán ngay" };
        }
        ...
        public static void splitService(CSS_OP_BOOKING_SGEV_WEB_EX dto)
        {
            if (!string.IsNullOrWhiteSpace(dto.TT_BO_SUNG))
            {
                ...
                for (int i = 0; i < listSplit.Length; i++)
                {
                    ...
                    if (listSplitChiTiet[0].Trim().Equals("BAO_PHAT"))
                    {
                        dto.BAO_PHAT = "V";
                    }
                    if (listSplitChiTiet[0].Trim().Equals("PHAT_TAN_TAY"))
                    {
                        dto.PHAT_TAN_TAY = "V";
                    }
                    if (listSplitChiTiet[0].Trim().Equals("KHAC"))
                    {
                        dto.KHAC = "V";
                    }
                    if (listSplitChiTiet[0].Trim().Equals("DONG_KIEM"))
                    {
                        dto.DONG_KIEM = "V";
                    }
                }
